...
namespace lt567 {
	...
	bool checkInclusion(string s1, string s2) {
		...
		/*	if (s2[offset] == s2[i])
			...
			else if (m1[s2[offset]] == 0) count++;

			if (m1[s2[i]]-- == 0) {
				count--;
			}
			else if (m1[s2[i]] == 0)count++;
			offset++;
		}
		return false;
	}
...
namespace lt765 {
	int minSwapsCouples(vector<int>& row) {
		unionfind u(row.size());
		map<int, int> cmap;
		for (int i = 0; i < row.size(); i++) {
			cmap[row[i]] = i;
		}
		for (int i = 0; i < row.size(); i+=2) {
			if ((row[i]+row[i]%2==0?1:-1)==row[i+1]||u.connect(row[i+1], cmap[row[i] + row[i] % 2 == 0 ? 1 : -1])) {
				
			}
			else {
				u.un_ion(row[i + 1], cmap[row[i] + row[i] % 2 == 0 ? 1 : -1]);
			}
		}
		return 0;
	}
	void test() {
		vector<int> te = { 0, 2, 1, 3 };
		minSwapsCouples(te);
	}
}
// namespace lt{
...
namespace lt556{
	vector<vector<int>> matrixReshape(vector<vector<int>>& nums, int r, int c) {
        if(nums.empty()||nums[0].empty()||r==0||c==0)
        return nums;
        int row=nums.size();
        int cow=nums[0].size();
        if(row*cow!=r*c)
